...
import com.mahdi.car.messenger.Bitmaps;
import com.mahdi.car.messenger.BuildVars;
...
public class CameraController implements MediaRecorder.OnInfoListener
{
            ...
            try {
                if (cameraInfos == null) {
                    ...
                    for (int cameraId = 0; cameraId < count; cameraId++) {
                        ...
                        //                            if (App.mainInterfacePaused && App.externalInterfacePaused) {
                        ...
                        for (int a = 0; a < list.size(); a++) {
                            ...
                            if (!"samsung".equals(Build.MANUFACTURER) || !"jflteuc".equals(Build.PRODUCT) || size.width < 2048) {
                                ...
                                if (BuildVars.LOGS_ENABLED) {
                                    LOG.d("picture size = " + size.width + " " + size.height);
                                }
                            }
                        }
                ...
                AndroidUtilities.run(() -> {
                    ...
                    if (!withDelay && "APP_PAUSED".equals(e.getMessage())) {
                        AndroidUtilities.run(() -> initCamera(onInitRunnable, true), 1000);
                    }
                });
    ...
    public boolean takePicture(final File path, final CameraSession session, final Runnable callback)
    {
        ...
        try {
            camera.takePicture(null, null, (data, camera1) -> {
                Bitmap bitmap = null;
                int size = (int) (AndroidUtilities.getPhotoSize() / AndroidUtilities.mDensity);
    ...
    public void openRound(final CameraSession session, final SurfaceTexture texture, final Runnable callback, final Runnable configureCallback)
    {
        if (session == null || texture == null) {
            if (BuildVars.LOGS_ENABLED) {
                LOG.d("failed to open round " + session + " tex = " + texture);
            }
            return;
        }
        threadPool.execute(() -> {
            ...
            try {
                if (BuildVars.LOGS_ENABLED) {
                    LOG.d("start creating round camera session");
                }
                if (camera == null) {
                    camera = session.cameraInfo.camera = Camera.open(session.cameraInfo.cameraId);
                }
                ...
                camera.startPreview();
                if (callback != null) {
                    AndroidUtilities.run(callback);
                }
                if (BuildVars.LOGS_ENABLED) {
                    LOG.d("round camera session created");
                }
            } catch (Exception e) {
                session.cameraInfo.camera = null;
                if (camera != null) {
    ...
    public void recordVideo(final CameraSession session, final File path, final VideoTakeCallback callback, final Runnable onVideoStartRecord, boolean b)
    {
        ...
        Log.e("recordVideo", "recordVideo: " + "1");
                ...
                if (camera != null) {
                    ...
                    try {
                        ...
                        if (onVideoStartRecord != null) {
                            AndroidUtilities.run(onVideoStartRecord);
                        }
                        ...
                        recorder = null;
                        LOG.e(e);
                        Log.e("recordVideo", "error " + e.toString());
                    }
                }
            } catch (Exception e) {
                LOG.e(e);
    ...
    private void finishRecordingVideo()
    {
        ...
        String fileName = Integer.MIN_VALUE + "_" + 1366 + ".jpg";
