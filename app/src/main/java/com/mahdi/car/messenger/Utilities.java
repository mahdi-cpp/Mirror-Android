...
public class Utilities
{
    ...
    private static volatile DispatchQueue markerQueue = new DispatchQueue("markerQueue");
    ...
    /*
    public static void addMarker(Post post)
    {
        markerQueue.postRunnable(new Runnable()
        {
            @Override public void run()
            {
                ...
                try {
                    FileLocation Cover = null;

                    if (ali.medias.size() > 0) {
                        Cover = new TL_fileLocation();
                        Cover.local_id = ali.medias.get(0).local_id;
                        Cover.file_id = ali.medias.get(0).file_id;
                        Cover.volume_id = Integer.MIN_VALUE;
                        Cover.dc_id = Integer.MIN_VALUE;
                    }

                    if (Cover != null) {
                        File path = FileLoader.getPathToAttach(Cover, true);
                        key = path.getAbsolutePath();
                        BitmapDrawable bitmapDrawable = ImageLoader.getInstance().getImageFromMemory(key);
                        if (bitmapDrawable != null) {
                            Bitmap finalResult = bitmapDrawable.getBitmap();
                            AndroidUtilities.runOnUIThread(new Runnable()
                            {
                                @Override public void run()
                                {
                                    if (finalResult != null)
                                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.mapMarkerBitmap, ali, finalResult);
                                }
                            });
                            return;
                        }
                    ...
                    Drawable drawable = Application.applicationContext.getResources().getDrawable(R.drawable.livepin);
                    ...
                    if (Cover != null) {
                        bitmap = ImageLoader.loadBitmap(key, null, AndroidUtilities.dp(100), AndroidUtilities.dp(100), true);
                ...
                if (next) {
                    ImageLoader.getInstance().putImageToCache(new BitmapDrawable(result), key);
                }
